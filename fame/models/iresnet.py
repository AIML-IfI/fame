"""IResNet backbones used by the AdaFace face recognition models.

Vendored from the official AdaFace release (Kim et al., CVPR 2022) with two
changes relative to the copy in the original code base:

* ``adaface.py`` and ``adaface_norm.py`` were byte-identical apart from an L2
  normalization at the end of ``Backbone.forward``.  That option now lives in
  ``FaceEmbeddingModel(l2_normalize=...)`` and this file holds one backbone.
* The old ``Adaface_model.forward`` computed ``out = self.norm(x)`` and then
  ``out = self.backbone(x)``, discarding the normalized tensor and feeding raw
  [0, 1] pixels to a network trained on [-1, 1] inputs.  Normalization is now
  handled once, by ``NormalizedModel``.
"""
